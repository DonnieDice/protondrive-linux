...
export const importHmacKey = async (
    ...
    return crypto.subtle.importKey('raw', key.buffer as ArrayBuffer, { name: 'HMAC', hash: HASH_ALGORITHM }, false, keyUsage);
};
...
export const computeHmacSignature = async (key: HmacCryptoKey, data: Uint8Array) => {
    const signatureBuffer = await crypto.subtle.sign({ name: 'HMAC', hash: HASH_ALGORITHM }, key, data.buffer as ArrayBuffer);
...
export const verifyData = async (key: HmacCryptoKey, signature: Uint8Array, data: Uint8Array) => {
    return crypto.subtle.verify({ name: 'HMAC', hash: HASH_ALGORITHM }, key, signature.buffer as ArrayBuffer, data.buffer as ArrayBuffer);
};
