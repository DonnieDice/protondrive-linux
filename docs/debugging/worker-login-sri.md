...
- `scripts/ci/check-login-routing-regressions.sh` is run by the `Login/2FA Routing Regression Checks` workflow job and fails if the broad "left captcha page" completion path or the deep `/u/<id>/` redirect pattern is reintroduced.
