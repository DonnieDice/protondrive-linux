...
DISTRO_PATCH="$PROJECT_ROOT/patches/appimage/${DISTRO_ID}.patch"
if [ -f "$DISTRO_PATCH" ]; then
    echo "Applying AppImage/${DISTRO_ID} distro patch..."
    git apply "$DISTRO_PATCH" || echo " Already applied or failed"
else
    echo "No AppImage/${DISTRO_ID}.patch found — building with base code only"
fi
