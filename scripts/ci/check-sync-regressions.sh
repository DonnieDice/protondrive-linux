...
require_pattern src-tauri/src/main.rs "read_sync_file" "local-to-remote sync bridge needs a safe file-read command"
require_pattern src-tauri/src/main.rs "get_sync_device_name" "local-to-remote sync bridge needs a stable Linux device name command"
require_pattern src-tauri/src/main.rs "MAX_SYNC_BRIDGE_FILE_BYTES" "sync bridge file reads must stay bounded"
require_pattern src-tauri/src/main.rs "validate_sync_relative_path" "sync bridge file reads must reject path traversal"
require_pattern src-tauri/src/main.rs "docs/sync/sync.md" "inline sync bridge comment must point maintainers to docs/sync/sync.md"
...
require_pattern README.md "docs/sync/sync.md" "README must link to sync operations documentation"
require_pattern docs/sync/sync.md "Current Status" "sync documentation must describe current status"
require_pattern docs/sync/sync.md "Known weak areas" "sync documentation must keep weak areas visible"
require_pattern docs/sync/sync.md "~/Pictures" "sync documentation must keep Pictures test plan visible"
require_pattern docs/sync/sync.md "live-sync://local-change" "sync documentation must record frontend event contract"
require_pattern docs/sync/sync.md "handle_remote_update" "sync documentation must record remote apply contract"
require_pattern docs/sync/login-sync-regression-runbook.md "right-side Proton app rail" "future Linux settings UI target must stay documented"
require_pattern docs/sync/login-sync-regression-runbook.md "quick-settings drawer" "initial Linux drawer target must stay documented"
require_pattern docs/sync/sync-database.md "Privacy model" "zero-trust sync metadata model must stay documented"
require_pattern docs/sync/sync-database.md "A tombstone is only created if the item exists in a known state" "sync docs must preserve conservative delete semantics"
require_pattern docs/sync/sync.md "createDevice\\(name, DeviceType\\.Linux\\)" "default remote device API model must stay documented"
require_pattern docs/sync/sync.md "rootFolderUid" "Proton Drive device rootFolderUid mapping must stay documented"
require_pattern docs/sync/sync.md "shareId" "Proton Drive device shareId mapping must stay documented"
require_pattern docs/sync/sync.md "read_sync_file" "local-to-remote upload bridge contract must stay documented"
require_pattern scripts/patch_drive_linux_sync_bridge.py "ProtonDriveLinuxSyncBridge" "WebClients sync bridge must stay documented"
require_pattern docs/sync/sync.md "My files" "extra mappings must target My files, not Computers"
require_pattern docs/sync/login-sync-regression-runbook.md "Do not start with the full" "Pictures safety warning must remain documented"
require_pattern docs/sync/login-sync-regression-runbook.md "Manual Sync Procedure" "manual sync procedure must stay documented"
require_pattern docs/sync/login-sync-regression-runbook.md "~/ProtonDrive" "manual sync procedure must use the default drive root"
require_pattern docs/sync/login-sync-regression-runbook.md "PROTONDRIVE_AUTO_SYNC_PATH" "manual sync procedure must document env override for extra mapping tests"
require_pattern docs/sync/login-sync-regression-runbook.md "\\[LiveSync\\] watcher active root=" "manual sync procedure must document watcher marker"
require_pattern docs/sync/login-sync-regression-runbook.md "source=default" "manual sync procedure must document normal default root startup marker"
require_pattern docs/sync/login-sync-regression-runbook.md "No test may be considered a sync pass from Drive/Photos API traffic alone" "manual sync procedure must reject API-only false positives"
require_pattern docs/sync/login-sync-regression-runbook.md "silent window where file changes occur but no .*\\[Sync\\].*\\[LiveSync\\]" "manual sync procedure must catch silent no-op sync"
...
require_pattern scripts/patch_drive_linux_drawer.py "Proton Drive Linux owns native sync/settings controls in this rail" "drawer visibility override must remain documented in source"
...
require_pattern scripts/build-webclients.sh "patch_drive_linux_sync_bridge\\.py" "WebClients sync bridge patch must run during every WebClients build"
...
require_pattern scripts/patch_drive_linux_drawer.py "applications/drive/src/app/components/layout/DriveWindow\\.tsx" "Linux drawer fallback must target current DriveWindow location"
require_pattern scripts/patch_drive_linux_drawer.py "showDrawerSidebar: true" "Linux drawer fallback must show rail by default"
require_pattern scripts/patch_drive_linux_sync_bridge.py "live-sync://local-change" "sync bridge fallback must listen to native local-change events"
require_pattern scripts/patch_drive_linux_sync_bridge.py "DeviceType\\.Linux" "sync bridge must create/find a Linux device"
require_pattern scripts/patch_drive_linux_sync_bridge.py "uploadManager\\.upload" "sync bridge must use WebClients upload manager"
require_pattern scripts/patch_drive_linux_sync_bridge.py "read_sync_file" "sync bridge must read local files through the safe Tauri command"
require_pattern scripts/patch_drive_linux_sync_bridge.py "get_sync_device_name" "sync bridge must use the native sanitized device name"
...
require_absent docs/sync/sync.md "Computers/<PC name>" "docs must not describe Computers as a path prefix"
