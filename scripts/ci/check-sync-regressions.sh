...
require_pattern docs/sync.md "Known weak areas" "sync documentation must keep weak areas visible"
...
require_pattern docs/login-sync-regression-runbook.md "right-side Proton app rail" "future Linux settings UI target must stay documented"
require_pattern docs/login-sync-regression-runbook.md "quick-settings drawer" "initial Linux drawer target must stay documented"
require_pattern docs/sync-database.md "Privacy model" "zero-trust sync metadata model must stay documented"
require_pattern docs/sync-database.md "A tombstone is only created if the item exists in a known state" "sync docs must preserve conservative delete semantics"
...
require_pattern docs/sync.md "read_sync_file" "local-to-remote upload bridge contract must stay documented"
...
require_pattern docs/login-sync-regression-runbook.md "Do not start with the full" "Pictures safety warning must remain documented"
