...
require_pattern src-tauri/src/main.rs "DEFAULT_REMOTE_DEVICE_PARENT_DIR" "remote target must be device-scoped under Computers"
require_pattern src-tauri/src/main.rs "default_remote_device_folder_path" "default remote device folder mapping must stay explicit"
...
require_pattern src-tauri/src/sync_db.rs "remote_path_hash" "sync metadata must store hashed remote mapping paths"
...
require_pattern docs/sync.md "Computers/<PC name>" "default remote device folder model must stay documented"
