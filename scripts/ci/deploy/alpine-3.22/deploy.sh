#!/usr/bin/env bash
# Deploy + smoke-test the alpine322 package on its VM (192.168.1.126).
# Distro-specific install quirks live in install_pkg() below — edit here, not
# in _common.sh. Called by .gitlab/workflows/verify/alpine-3.22.yml.
set -euo pipefail
...
deploy_run "alpine322" "$VM_IP" "$PKG_GLOB" install_pkg
