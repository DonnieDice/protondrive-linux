...
This is the complete project context for AI agents. It explains WHY decisions were made, WHAT the architecture is, HOW to build it, and ALL tasks to completion. This is NOT user documentation - that's in README.md.

**Purpose**: Help AI agents understand project context, implement features correctly, and track progress to completion.

---

## AGENT LOGS & OUTCOMES

### The `.agent_logs/` Directory

**Location**: `/.agent_logs/`

**Purpose**: Store session outcomes, decision logs, and progress reports generated by AI agents during development.

### What Goes in `.agent_logs/`

1. **Session Outcomes** - Summaries of completed tasks
   - Test fix reports (e.g., `TEST_FIXES_COMPLETE.md`)
   - CI/CD pipeline fixes (e.g., `CI_CD_READY.md`)
   - Feature implementation summaries
   - Bug fix reports

2. **Progress Milestones** - Achievement documentation
   - Test coverage improvements (e.g., `100_PERCENT_PASSING.md`)
   - Phase completions (e.g., `PHASE_2_P0_COMPLETE.md`)
   - Performance optimizations

3. **Decision Records** - Why choices were made
...
- `{TOPIC}_{STATUS}.md` - e.g., `TEST_FIXES_COMPLETE.md`
- `{FEATURE}_{OUTCOME}.md` - e.g., `AUTH_IMPLEMENTATION_SUCCESS.md`
- `{PHASE}_{MILESTONE}.md` - e.g., `PHASE_2_P0_COMPLETE.md`

### When to Create Log Files

AI agents MUST create a log file when:
1. ✅ **Completing a major task** - Document what was done and results
2. ✅ **Fixing critical issues** - Explain problem, solution, and verification
3. ✅ **Making architectural decisions** - Record reasoning and alternatives
4. ✅ **Achieving milestones** - Document progress (e.g., 100% tests passing)
5. ✅ **Encountering blockers** - Document issues and attempted solutions
...
## Files Modified
1. `path/to/file.ts` - Brief description of changes
2. `path/to/another.ts` - Brief description of changes
...
## Notes
- Important context or learnings
- Warnings or caveats
...
**For AI Agents**:
1. **Before starting work**: Read recent logs in `.agent_logs/` to understand what was done
2. **During work**: Reference logs for context on previous decisions
3. **After completing work**: Create a new log file documenting outcomes
4. **When stuck**: Check logs for similar problems and solutions
...
# Read the most recent test fix log
cat .agent_logs/TEST_FIXES_COMPLETE.md

# Create a new outcome log
cat > .agent_logs/FEATURE_X_COMPLETE.md << 'EOF'
...
✅ **DO**:
- Include concrete metrics (tests passing, performance, etc.)
...
❌ **DON'T**:
- Create vague or generic summaries
- Skip documenting major changes
- Include sensitive information (passwords, tokens)
- Duplicate information already in git commits
- Leave incomplete or draft logs
...
**P0: Documentation Structure**
- [ ] Create README.md in all src/ subdirectories
...
2. Check `.agent_logs/` for recent session outcomes
3. Review relevant code in `src/`
4. Check existing tests in `src/__tests__/`

### During Work
1. Follow coding standards (see above)
...
3. Create outcome log in `.agent_logs/`
...
# Create outcome log
cat > .agent_logs/OUTCOME_NAME.md << 'EOF'
# Title - Status
...
EOF
```
...
**Version**: 5.1  
**Status**: ACTIVE - Complete Context with Agent Logs System